...
def evaluate(
    ...
    for label in range(generator.num_classes()):
        ...
        results.append({'label': generator.label_to_name(label), 'ap': average_precision, 'na': num_annotations})
    ...
    if not test:
        ...
        for label in range(generator.num_classes()):
            ...
            if not test:
                wandb.log({'label': label_name, 'average_precision': average_precisions[label][0], 'precision': precision[-1],
                           'recall': recall[-1]}, step=epoch)
    ...
    return average_precisions, results
